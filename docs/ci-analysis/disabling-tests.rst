...
If the test is written in `mocha`_ or `jest`_ (easy way to tell is if it uses ``describe`` and ``it`` syntax), you can tack on ``.skip()`` at the end of the ``describ``e keyword for the test case that is failing.
